...
# Installation
To install onto a BTTPi, use files in the `Biqu` folder. To install onto a RaspberryPi or similar, use files in the `RaspberryPi` folder. The only difference are paths in `tool_watcher.service` and `tool_watcher.py`.
